	...
	$.ajax({
		...
		success : function(data) {
			if (data.code == 0) {
				...
				// 设置显示
				commentNum = parseInt($('#commentsNum').text().trim()) + 1;
				$('#commentsNum').html(
						'<i class="fa fa-comments"></i> ' + commentNum);
			} else {
				$.modalMsg("评论失败！", "error");
				flag = -1;
				blogCommentId = '';
...
(function($) {
	function crateCommentInfo(obj, flag) {
		...
		var el = "<div class='comment-info'><div class='headerimg'><img class='img' src='"
				+ obj.replyHeadImg
				+ "' onerror=\"this.src='/img/blog/default1.jpg'\"></div><div class='comment-right'><div class='commenterinfo'><input value='"
				+ obj.blogCommentId
	...
	function createReplyComment(reply, flag) {
		var html = "";
		if (flag == 0) {
			html = "<span class='reply-list-btn'>回复</span><span class='reply-comment-like fa fa-thumbs-o-up' id='comment-like-"
					+ reply.blogCommentId
					+ "'> "
	...
	$.fn.addCommentList = function(options) {
		...
		if (option.data.length > 0) {
			...
			// 监听点赞
			$(".reply-comment-like").click(function() {
				var id = this.id;
