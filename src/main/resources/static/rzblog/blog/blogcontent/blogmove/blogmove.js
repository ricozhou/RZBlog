...
$(function() {
			...
			.bind(
					...
					function() {
						...
						if (moveMode == 0) {
							...
							// 去除一个不可以列表获取的选项
							document.getElementById("moveWebsite").options
									.remove(7);
							document.getElementById("moveWebsite").options
									.remove(8);
						} else if (moveMode == 1) {
							...
							// 添加一个不可以列表获取但是单篇可以获取的选项
							document.getElementById("moveWebsite").options
									.add(new Option("脚本之家", "脚本之家"));
							document.getElementById("moveWebsite").options
									.add(new Option("阿里云栖社区", "阿里云栖社区"));
	...
	$('#moveWebsite').bind('change', function() {
		...
		if ($('#moveMode').val() == 0) {
			var preUrl = "";
			if (moveWebsite == 'CSDN') {
				preUrl = 'https://blog.csdn.net/';
			} else if (moveWebsite == '博客园') {
				preUrl = 'http://www.cnblogs.com/';
			} else if (moveWebsite == '今日头条') {
				preUrl = 'https://www.toutiao.com/c/user/';
			} else if (moveWebsite == '简书') {
				preUrl = 'https://www.jianshu.com/u/';
			} else if (moveWebsite == '开源中国') {
				preUrl = 'https://my.oschina.net/u/';
			} else if (moveWebsite == '微信公众号') {
				preUrl = '微信号：如（CSDN）：CSDNnews';
			} else if (moveWebsite == '51CTO') {
				preUrl = 'http://blog.51cto.com/';
			}
			$('#moveWebsiteInput').removeClass('col-sm-5');
			$('#moveWebsiteInput').addClass('col-sm-2');
			$('#moveWebsiteUrl').css('display', 'block');
			$('#moveWebsiteUrl').html(preUrl);
...
function moveRun() {
	...
	if (moveMode == 0) {
		if (moveWebsiteId == '微信公众号') {
			if (moveNum > 10) {
				$.modalAlert('公众号文章最多10篇！', modal_status.WARNING);
				return;
			}
		}
	} else if (moveMode == 1) {
