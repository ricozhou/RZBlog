...
$().ready(
		function() {
			...
			}else if(basicsetArticleEditor==2){
				// 使用simditor编辑器
			...
			$.ajax({
				cache : true,
				type : "GET",
				url : ctx + "blog/blogtemplate/selectBlogTemplateList",
				data:{
...
// $.modalAlert("系统错误", modal_status.FAIL);
				...
				success : function(data) {
					// 动态显示模板
					var newOption;
					$("#blogTemplateId").find("option").remove();
					newOption = document.createElement("option");
					newOption.text = '暂不选择';
					newOption.value = '';
					document.getElementById("blogTemplateId").add(newOption);
					if (data != null) {
						for (var i = 0; i < data.length; i++) {
							newOption = document.createElement("option");
							newOption.text = data[i].templateName;
							newOption.value = data[i].blogTemplateId;
							document.getElementById("blogTemplateId").add(
									newOption);
						}
					} else {
// $.modalAlert(data.msg, modal_status.FAIL);
...
function getTemplateContentById(id){
	...
	$.ajax({
		cache : true,
		type : "GET",
		url : ctx + "blog/blogtemplate/selectBlogTemplateById",
		data:{
			"blogTemplateId":id
		},
		async : false,
		...
		success : function(data) {
			if(data!=null){
				if (basicsetArticleEditor == 0) {
					// html
					blogTemplateContent=data.content;
				} else if (basicsetArticleEditor == 1) {
					// markdown
					blogTemplateContent=data.contentMd;
				}
			}
		}
	});
	
	return blogTemplateContent;
}
...
// $("#bookClassification2").hide();
	...
	// 监听模板选择选择
	$('#blogTemplateId').bind('change', function() {
		var blogTemplateId = $(this).val();
		var blogTemplateContent='';
		if (blogTemplateId != null&&blogTemplateId!='') {
			blogTemplateContent=getTemplateContentById(blogTemplateId);
		} 
		
		// 赋值到编辑器
		if(blogTemplateContent!=null){
			if (basicsetArticleEditor == 0) {
				// html
				// 追加到前面
				$('#content_sn').summernote('code', blogTemplateContent+"\r\n"+$("#content_sn").summernote('code'));
			} else if (basicsetArticleEditor == 1) {
				// markdown
				simplemde.value(blogTemplateContent+"\r\n"+simplemde.value())
			} else if (basicsetArticleEditor == 2) {
				// simeditor
				simeditor.setValue(blogTemplateContent+"\r\n"+simeditor.getValue())
			}
		}
