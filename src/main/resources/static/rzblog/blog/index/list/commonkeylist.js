...
$(document).ready(function() {
	title = basicsetTitle + "-" + typeName + "（" + param + "）";
	// 设置信息
	if (param != null && param != '') {
		...
		document.getElementById("n2").href = typeUrl + param;
		document.getElementById("n2").innerText = typeName + "（" + param + "）";
...
function initType() {
	if (author != null && author != '') {
		typeName = '作者';
		param = author;
		typeUrl = '/rzblog/blogauthor/?author=';
		postData = {
			'author' : author,
			pageSize : pageSize,
			pageNum : pageNum,
		...
		postUrl = '/rzblog/blogauthorList';
	} else if (blogType != null && blogType != '') {
		typeName = '分类';
		param = blogType;
		typeUrl = '/rzblog/blogarticletype/?blogtype=';
		var t = 0;
		if (blogType == '文章') {
			t = 0;
		} else if (blogType == '软件') {
		...
		postUrl = '/rzblog/blogarticletypeList';
	} else if (type != null && type != '') {
		typeName = '类型';
		param = type;
		typeUrl = '/rzblog/blogtype/?type=';
		postData = {
			'type' : type,
			pageSize : pageSize,
			pageNum : pageNum,
		...
		postUrl = '/rzblog/blogtypeList';
	} else if (articleSource != null && articleSource != '') {
		typeName = '来源';
		param = articleSource;
		typeUrl = '/rzblog/blogsource/?source=';
		postData = {
		...
		postUrl = '/rzblog/blogsourceList';
	} else {
		typeName = '搜索';
		param = '其他';
		typeUrl = '/rzblog/search/?keyword=';
		postData = {
			status : 1,
			content : param,
			pageSize : pageSize,
	...
	$
			.ajax({
				...
				success : function(data) {
					var rows = data.rows;
					if (rows == null || rows == '') {
						var nodataHtml = '<p class="news_title">暂无文章，请查看其他'
								+ typeName + '！</p>';
...
function getHeightKeyword(html) {
	/*
	 * return html.split(keyword).join( '<span style="color:red;">' + keyword + '</span>');
	 */
	return html;
}
