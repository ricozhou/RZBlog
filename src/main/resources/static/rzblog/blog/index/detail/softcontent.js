				...
				function() {
					...
					if (blogcolumn != null && blogcolumn != '') {
						...
						if (blogcolumn.columnType == 'M') {
							...
							// 设置右侧位置信息
							document.getElementById("columnMsg").innerHTML = '<i class="fa fa-map-marker"></i> 您现在的位置是：<a href="/rzblog">首页</a> > <a href="/rzblog/column'
									+ blogcolumn.url
									+ '">'
						...
						// 设置右侧位置信息
						document.getElementById("columnMsg").innerHTML = '<i class="fa fa-map-marker"></i> 您现在的位置是：<a href="/rzblog">首页</a> > ';
					}
					...
					var bloginfoHtml = '<li class="author"><i class="fa fa-user"></i> '
						...
						+ '" target="_blank">'
							...
							+ '</li><li class="commentNum" id="commentsNum"><i class="fa fa-comments"></i> ' + (blogcontent.commentsNum<0?'0':blogcontent.commentsNum)
					...
					// 设置标题
					var blogTypeHtml='';
					if(blogcontent.blogType==0){
						blogTypeHtml='<a href="/rzblog/blogarticletype/?blogtype=文章" title="文章" target="_blank" style="margin-left:5px;"><div class="ui red label horizontal" style="background-color:#2185d0!important">文章</div></a>';
					}else if(blogcontent.blogType==1){
						blogTypeHtml='<a href="/rzblog/blogarticletype/?blogtype=软件" title="软件" target="_blank" style="margin-left:5px;"><div class="ui red label horizontal" style="background-color:#32aa66!important">软件</div></a>';
					}else if(blogcontent.blogType==2){
						blogTypeHtml='<a href="/rzblog/blogarticletype/?blogtype=影视" title="影视" target="_blank" style="margin-left:5px;"><div class="ui red label horizontal" style="background-color:#b5cc18!important">影视</div></a>';
					}
					
						var titleHtml = '<a href="/rzblog/blogtype/?type='
							+ blogcontent.type
							+ '" title="'
							+ blogcontent.type
					...
					// 设置软件相关信息
					$('#softCreateTime').text(new Date(blogcontent.softcontent.createTime)
					.format("yyyy-MM-dd"));
					
					// 获取更新链接，然后获取更新信息，更新官网链接，下载链接文档链接，急速下载链接等
					handleSoftUpdate();
					
					// 设置协议信息
					document.getElementById("softLicense")
					.insertAdjacentHTML('afterBegin',
							'<a href="'+getLicenseUrlByIndex(blogcontent.softcontent.softLicense)+'" target="_blank">'+getLicenseNameByIndex(blogcontent.softcontent.softLicense)+'</a>');
					...
					if (blogset.basicsetShowAdvertisement == 0
						...
						if (blogadvertisementList != null
							...
							if(blogadvertisementList.length > 1){
								...
								$('#banner4').removeClass('fader2');
								$('#banner4').addClass('faderNew2');
								// 左右按钮隐藏
								$('.fader2_controls').css('display','none');
							}
							$("#blogconcentAd").css('display', 'block');
...
function sendComment(){
	...
	if(obj.replyQqNum!=null&&obj.replyQqNum!=''){
		obj.replyHeadImg="http://q1.qlogo.cn/g?b=qq&nk="+obj.replyQqNum+"&s=100";
	}else{
...
var num=0;
// 增加浏览和点赞量
function addBlogIndexNum(flag) {
	// flag0是浏览量，1是点赞量
	$.ajax({
		cache : true,
		type : "POST",
		url : "/rzblog/blogcontent/addBlogIndexNum",
		data : {
			"cid" : blogcontent.cid,
			"flag" : flag
		},
		...
		success : function(data) {
			if (data.code == 0) {
				if(flag==1){
					...
					// 更改下载量
					dnum=parseInt($('#downloadNum').html())+1;
					$('#downloadNum').html(dnum);
				}
...
// 处理软件更新链接
function handleSoftUpdate(){
	
	var softWebsite=blogcontent.softcontent.softWebsite;
	var softDocUrl=blogcontent.softcontent.softDocUrl;
	var softCodeUrl=blogcontent.softcontent.softDownUrl;
	var softDownUrl=blogcontent.softcontent.softDownUrl;
	
	if(blogcontent.softcontent.softUpdateUrl!=null&&blogcontent.softcontent.softUpdateUrl!=''){
		// 更新
		// 切换链接，不再使用/softpage/{softAppId}为/latest/{softAppId}
		
		$.ajax({
			cache : false,
			type : "GET",
			url : blogcontent.softcontent.softUpdateUrl,
			data : {
			},
			async : false,
			error : function(request) {
			...
			success : function(res) {
				if (res&&res.data) {
					if(res.data.softWebsiteUrl!=null&&res.data.softWebsiteUrl!=""){
						softWebsite=res.data.softWebsiteUrl;
					}
                    if(res.data.softFileDownUrl!=null&&res.data.softFileDownUrl!=""){
                    	softDownUrl=res.data.softFileDownUrl;
					}
// if(res.data.softDocUrl!=null&&res.data.softDocUrl!=""){
// softDocUrl=res.data.softDocUrl;
// }
                    if(res.data.softSourceCodeUrl!=null&&res.data.softSourceCodeUrl!=""){
                    	softCodeUrl=res.data.softCodeUrl;
                    }
				}
			}
		});
	...
	if(softWebsite==null||softWebsite==''){
		$("#softWebsiteb").css('display', 'none');
	}else{
		document.getElementById("softWebsiteb").href =softWebsite
	}
	if(softDocUrl==null||softDocUrl==''){
		$("#softDocUrlb").css('display', 'none');
	}else{
		document.getElementById("softDocUrlb").href =softDocUrl
	}
	if(softCodeUrl==null||softCodeUrl==''){
		$("#websoftDownUrlb").css('display', 'none');
	}else{
		document.getElementById("websoftDownUrlb").href =softCodeUrl
	}
	if(softDownUrl==null||softDownUrl==''){
		$("#softDownUrlb").css('display', 'none');
	}else{
		document.getElementById("softDownUrlb").href =softDownUrl
	}
}
