...
$(function() {
	...
	function menuItem() {
				...
				function() {
					if ($(this).data('id') == dataUrl) {
						if (!$(this).hasClass('active')) {
							...
							scrollToTab(this);
							// 显示tab对应的内容区
							$('.mainContent .RZBlog_iframe').each(
									function() {
										if ($(this).data('id') == dataUrl) {
											$(this).show().siblings(
													'.RZBlog_iframe').hide();
		...
		if (flag) {
			...
			// 添加选项卡对应的iframe
			var str1 = '<iframe class="RZBlog_iframe" name="iframe'
					+ dataIndex + '" width="100%" height="100%" src="'
					+ dataUrl + '" frameborder="0" data-id="' + dataUrl
					+ '" seamless></iframe>';
			$('.mainContent').find('iframe.RZBlog_iframe').hide().parents(
					'.mainContent').append(str1);
			// 加载遮盖处层
			//暂时不使用遮盖层
			// $.modelLoading("数据加载中，请稍后...");
	...
	function closeTab() {
		...
		if ($(this).parents('.menuTab').hasClass('active')) {
			...
			if ($(this).parents('.menuTab').next('.menuTab').size()) {
				...
				$('.mainContent .RZBlog_iframe').each(function() {
					if ($(this).data('id') == activeId) {
						$(this).show().siblings('.RZBlog_iframe').hide();
						return false;
					}
				});
	...
	function closeOtherTabs() {
		$('.page-tabs-content').children("[data-id]").not(":first").not(
				".active").each(
				function() {
					$('.RZBlog_iframe[data-id="' + $(this).data('id') + '"]')
							.remove();
					$(this).remove();
				});
	...
	function refreshTab() {
		...
		var target = $('.RZBlog_iframe[data-id="' + currentId + '"]');
