/**
 * 通用方法封装处理 Copyright (c) 2018 rzblog
 */
$(function() {
			// 打开遮罩层
			$.modelLoading = function(message) {
				$
						.blockUI({
			...
			$.modalMsg = function(content, type) {
				if (type != undefined) {
					...
					$(".layui-layer-msg").find('i.' + icon).parents(
							'.layui-layer-msg').addClass(
							'layui-layer-msg-' + type);
				} else {
					layer.msg(content);
				}
			}
		...
		$('.bootstrap-table').bootstrapTable({
			...
			queryParams : function(params) {
				...
				if (params.sort == 'cid') {
					...
					sortName = 'browse_num';
				} else if (params.sort == 'downloadNum') {
					sortName = 'download_num';
