...
/** 对jquery的ajax方法再次封装 */
_ajax3 = function(url, data, type, callback) {
	var config = {
		url : url,
		type : type,
		dataType : "json",
...
/** 返回结果处理 */
function simpleSuccess2(result) {
	if (result.code == web_status.SUCCESS) {
		$.modalMsg(result.msg, modal_status.SUCCESS);
		$.refreshTable();
	} else {
...
function createMenuItem(dataUrl, menuName) {
			...
			function() {
				if ($(this).data('id') == dataUrl) {
					if (!$(this).hasClass('active')) {
						...
						// 显示tab对应的内容区
						$('.mainContent .RZBlog_iframe', topWindow).each(
								function() {
									if ($(this).data('id') == dataUrl) {
										$(this).show().siblings(
												'.RZBlog_iframe').hide();
	...
	if (flag) {
		...
		// 添加选项卡对应的iframe
		var str1 = '<iframe class="RZBlog_iframe" name="iframe' + dataIndex
				+ '" width="100%" height="100%" src="' + dataUrl
				+ '" frameborder="0" data-id="' + dataUrl
				+ '" seamless></iframe>';
		$('.mainContent', topWindow).find('iframe.RZBlog_iframe').hide()
...
// 设置下拉框默认值
function setSelectDefault(data, id) {
 ...
 *            callback 回调函数
 */
function dynamicLoadJsFileOnBody(url, callback) {
