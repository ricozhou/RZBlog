...
import com.rzblog.project.blog.blogbrowe.utils.BlogbroweUtils;
...
import com.rzblog.project.blog.blogoverview.domain.CityBroweNum;
import com.rzblog.project.blog.blogoverview.domain.CityMap;
...
public class BlogoverviewServiceImpl implements IBlogoverviewService {
	...
	public Blogoverview selectBlogoverview(Blogoverview blogoverview) {
		...
		// 总评论数
		blogoverview.setAllCommentNum(blogcommentService.selectAllBlogCommentNum());
		...
		// 日评论数
		blogoverview.setDayCommentNum(blogcommentService.selectDayBlogCommentNum());
		blogoverview.setDayCommentNumPer(BlogbroweUtils.getBlogMsgNumPer(blogoverview.getDayCommentNum(),
				blogcommentService.selectDayBlogCommentNumPre()));
		...
		// 月评论数
		blogoverview.setMonthCommentNum(blogcommentService.selectMonthBlogCommentNum());
		blogoverview.setMonthCommentNumPer(BlogbroweUtils.getBlogMsgNumPer(blogoverview.getMonthCommentNum(),
				blogcommentService.selectMonthBlogCommentNumPre()));
		...
		// 年评论数
		blogoverview.setYearCommentNum(blogcommentService.selectYearBlogCommentNum());
		blogoverview.setYearCommentNumPer(BlogbroweUtils.getBlogMsgNumPer(blogoverview.getYearCommentNum(),
				blogcommentService.selectYearBlogCommentNumPre()));
	...
	private Integer[] getWeekCommentNumArray() {
		...
		for (int i = 0; i < weekCommentNumArray.length; i++) {
			weekCommentNumArray[i] = blogcommentService
					.selectBlogCommentNumPreByDayNum(weekCommentNumArray.length - i - 1);
		}
